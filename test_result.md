...
agent_communication:
    ...
    - agent: "main"
      message: "🔧 MAJOR UX FLOW RESTRUCTURE & BUG FIX COMPLETED: 1) Fixed disqualification message bug - 'GLP-1 Therapy Not Recommended' message now ONLY appears for GLP-1 (weight-loss category) services when patient is ineligible. For Hormone Health and Hair Loss services, no disqualification logic is applied - patients proceed to booking regardless of answers. 2) Restructured booking flow to match user requirements: NEW FLOW is Service Selection → Medical Questionnaire → Demographics/Booking → Payment (vs OLD FLOW: Demographics → Service Selection → Questionnaire → Payment). The questionnaire now appears IMMEDIATELY after service selection, and only after completing it (or if service doesn't require it) do patients see the demographics form. This prevents patients from entering personal info before determining eligibility. Updated MedicalQuestionnaire.jsx to add serviceCategory check for disqualification logic, and completely rewrote Booking.jsx with new step flow ('service' → 'questionnaire' → 'demographics' → 'processing')."
